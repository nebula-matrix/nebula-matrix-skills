...
2. **定位问题** - 在 `html_source/*_processed.html` 中查找对应章节锚点的 HTML 结构
3. **修复脚本** - 使用 Edit 能力修改 `post_process_html()` 中的 Pattern A0/A0b 正则，添加对新格式的匹配
4. **重新验证** - 重新运行转换，确认所有一级标题正确生成，验证通过

**常见修复场景**：
- 章节编号和标题在同一 `<font>` 标签内 → 添加 `(\d+)([A-Za-z]+)` 匹配模式
- 多个 `<a>` 锚点 → 使用 `(?:<a[^>]*></a>)*` 匹配零个或多个锚点
- 特殊字符标题 → 扩展字符匹配范围
