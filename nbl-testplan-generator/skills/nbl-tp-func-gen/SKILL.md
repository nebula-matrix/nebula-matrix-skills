...
2. 执行：`python parsers/md_parser.py {spec_md_path} > {output_dir}/spec_tree.json`
...
**操作：**
1. 执行：`python reg_to_json.py {reg_xlsx_path} -o {output_dir}/reg_info.json -m {module_name}`
...
8. **追加到结果：**
   - 将当前 Feature 的完整结构追加到 `testplan_func_raw.json` 的 `features` 数组
   - 继续处理下一个 Feature

**Phase 4-C: 生成 review 文件**
1. 所有 Feature 处理完成后，调用 `review_writer.py`：
   `python review/review_writer.py {output_dir}/testplan_func_raw.json {output_dir}/fs_reg_slv_review.md`
...
2. 执行：`python writers/combine_writer.py {output_dir}/testplan_func_raw.json skills/nbl-tp-func-gen/reference/template/testplan_template.xlsx {output_dir}/{module_name}_testplan_func.xlsx`
