...
```
输入：工作目录 + 章节信息（所有页面、预分配模板、内容密度）
  ↓
1. 审视模板分配：章内相邻页不得使用相同模板，如有冲突则调整
  ↓
2. 逐页生成（每页执行）：
   a. 选择模板 → 读取模板文件 → 替换占位符 → 保存 HTML
   b. Playwright 检测 + PPTX 预检
   c. 通过 → 下一页；不通过 → 修复重试（最多 3 次）
   d. 仍不通过 + 密度为"高" → 拆分为 2 页，通知主代理调整后续页码
  ↓
3. 全部页面生成后，章内批量复检
  ↓
...
**关键规则**：
- 模板选择和占位符替换参考 `reference/HTML页面生成说明.md`
- 文件命名：`{页码:02d}_{简短描述}.html`，保存到工作目录
- 检测脚本调用方法见 `reference/HTML页面生成说明.md` 的"页面检测与生成"
- PPT 转换限制（颜色格式、SPAN 样式、字体等）见同文档"PPT 转换技术注意事项"
