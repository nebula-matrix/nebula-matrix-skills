...
**改为章节级子代理**：每个章节调用一个独立的子代理（不再每页一个），**调用时必须传递完整的工作目录路径**作为参数。
...
2. **逐页生成**（对每页执行）
   a. 根据内容选择最合适的模板文件（优先使用预分配模板）
   b. 读取模板 → 替换所有 `{{占位符}}` → 保存为 `{页码:02d}_{简短描述}.html`
   c. 执行 Playwright 内容溢出检测：`uv run validate_with_playwright.py [页面路径]`
   d. 执行 PPTX 预检：`node pptx/generate_pptx.js --check [页面路径]`
   e. 任一检测失败 → 修复问题 → 重新检测，重复直到通过
   f. 如密度为"高"且反复溢出无法修复 → 拆分为 2 页，记录拆分通知主代理

3. **章内批量复检**
   - 所有页面生成后，再次执行批量 Playwright 检测和 PPTX 预检
   - 确认无遗漏、页码连续
...
**必须遵守的规则**：
- 文件命名：`{页码:02d}_{简短描述}.html`，保存到工作目录根目录
- 模板选择、占位符替换、检测脚本调用详见 `reference/HTML页面生成说明.md`
...
| 文档 | 内容 |
|------|------|
| `reference/HTML页面生成说明.md` | 模板清单与选择原则、章节子代理工作流、页面检测脚本调用、PPT 转换技术注意事项 |
