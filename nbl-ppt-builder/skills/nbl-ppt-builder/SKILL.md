...
**何时需要插入页面？**
- 章节子代理发现某页内容密度过高，单页无法承载时
- 用户反馈需要补充额外内容页时

**插页命名规则**
- 在原始页码后增加字母后缀：`{页码:02d}{字母}_{描述}.html`
- 第一次拆分：`03_市场.html` → 新增 `03a_细分市场.html`
- 第二次拆分：`03_市场.html` → 新增 `03b_市场数据.html`
- 多个子代理并行时，**各自只修改自己章节的插页，不修改其他章节的数字页码**

**排序与重排机制**
- 所有脚本按 `(数字, 字母)` 排序：`03` < `03a` < `03b` < `04`
- `merge_ppt_pages.py --renumber`：按排序后的文件顺序分配连续页码（1, 2, 3...），并替换每页内部显示的页码数字
- `generate_pptx.js`：同样支持字母后缀排序，确保 PPTX 输出顺序正确
